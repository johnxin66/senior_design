...
_DESTINATIONS_TO_COORDS = {"traffic": (480, 450), "tree": (425, 400), "concrete": (450, 710), "brick": (350, 680)}
_S_START = (500, 500)
...
barometer_data = _CLIENT.getBarometerData()
s = pprint.pformat(barometer_data)
print("barometer_data: %s" % s)

magnetometer_data = _CLIENT.getMagnetometerData()
...
def fly(dijkstra_waypoint_lists):
    for waypoints in dijkstra_waypoint_lists:
        ...
        for waypoint in reversed(waypoints):
            ...
            move_future = _CLIENT.moveToPositionAsync(new_waypoint[0], new_waypoint[1], new_waypoint[2], 10, drivetrain=airsim.DrivetrainType.ForwardOnly, yaw_mode=yaw_mode)
            while not move_future._set_flag:
                move_future._loop.start()
                distance_sensor_data = _CLIENT.getDistanceSensorData()
            ...
            current_pose = _CLIENT.simGetVehiclePose().position
            current_xy = current_pose.x_val, current_pose.y_val

        # Descend and hover for dropoff
        _CLIENT.hoverAsync().join()
        _CLIENT.moveToPositionAsync((waypoints[0][1] - 500), (waypoints[0][0] + _CORRECTION - 500), -5, 10).join()
        # state = _CLIENT.getMultirotorState()
        # print("state: %s" % pprint.pformat(state))
        print("Arrived")
...
user_input_destinations = ["traffic", "tree", "concrete", "brick"]
def main_fly(destinations):
    print("destinations")
    print(destinations)
    _CLIENT.reset()
    _CLIENT.enableApiControl(True)
    fly_to_user_input(destinations)
