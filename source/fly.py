...
_TEST_DIJKSTRA_WAYPOINTS, visited = dijkstra.searching()
print("_TEST_DIJKSTRA_WAYPOINTS")
print(_TEST_DIJKSTRA_WAYPOINTS)
...
# for waypoint in _WAY_POINTS:
#     client.moveToPositionAsync(waypoint[0], waypoint[1], waypoint[2], 5).join()
print("Flying...")
i = 0
current_xy = 0, 0, -50
for waypoint in reversed(_TEST_DIJKSTRA_WAYPOINTS):
    i += 1
    if i > 1 and i <= len(_TEST_DIJKSTRA_WAYPOINTS) - 1:
        prev = _TEST_DIJKSTRA_WAYPOINTS[i - 2]
        curr = _TEST_DIJKSTRA_WAYPOINTS[i - 1]
        next = _TEST_DIJKSTRA_WAYPOINTS[i]
...
client.hoverAsync().join()
client.moveToPositionAsync(-(_TEST_DIJKSTRA_WAYPOINTS[0][0] - 500), -(_TEST_DIJKSTRA_WAYPOINTS[0][1] + _CORRECTION - 500), -5, 10).join()
state = client.getMultirotorState()
print("state: %s" % pprint.pformat(state))
